...
class CustomerRecord(db.Model):
    ...
    def to_dict(self):
        ...
        return {
            ...
            'สถานะ': self.status,
    'วงเงินที่ต้องการ': f"{self.desired_credit_limit:,.2f}" if self.desired_credit_limit is not None else '-',
    'วงเงินที่อนุมัติ': f"{self.approved_credit_limit:,.2f}" if self.approved_credit_limit is not None else '-',
